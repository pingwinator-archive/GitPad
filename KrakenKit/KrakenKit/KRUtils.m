...
void KRLogInternal(const char * filename, unsigned int line, int dumpStack, NSString * format, ...)
{
	...
	@autoreleasepool {
		...
		if (enabledFilesSet == nil) {
			enabledFilesSet = [[NSSet alloc] initWithArray:[[NSUserDefaults standardUserDefaults] arrayForKey:KRLogEnabledFilenames]];
		}
		...
		NSString * outputFileName = [[NSUserDefaults standardUserDefaults] stringForKey:KRLogOutputFilename];
